...
publishing {
    repositories {
        maven {
            val releasesRepoUrl = "https://maven.pkg.github.com/RealAd"
            val snapshotsRepoUrl = "$buildDir/repos/snapshots"
            name = "deploy"
            url = uri(if (Ci.isReleaseVersion) releasesRepoUrl else snapshotsRepoUrl)
            credentials {
                username = System.getenv("GITHUB_USERNAME") ?: githubUsername
                password = System.getenv("GITHUB_PASSWORD") ?: githubPassword
            }
        }
    }
